...
describe('useAnecdote filtering', () => {
  ...
  it('returns filtered anecdote', () => {
    ...
    expect(result.current[0].content).toBe('testcontent2')
  })

  it('voting increments anecdote by 1', async () => {
    const mockAnecdote = [
      { id: '1', content: 'testcontent', votes: 1 }
    ]

    useAnecdoteStore.setState({ anecdotes: mockAnecdote })
    anecdoteService.update.mockResolvedValue({ ...mockAnecdote[0], votes: 2 })
