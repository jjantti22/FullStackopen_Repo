...
describe('Blog app', () => {
  ...
  describe('When logged in', () => {
    beforeEach(async ({ page }) => {
      await loginWith(page, 'mluukkai', 'salainen')
      await createBlog(page, 'test title', 'jokunen jossain', 'http://testi.com')
    })

    test('a new blog can be created', async ({ page }) => {
      await expect(page.getByText('a new blog test title by jokunen jossain added')).toBeVisible()
    })
    ...
    test('a blog can be removed', async ({ page }) => {
      ...
      await expect(page.getByText('test title jokunen jossain')).not.toBeVisible()
    })
